...
```bash
lambda-hello$ sam build --use-container
```
...
```bash
lambda-hello$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
lambda-hello$ sam local start-api
lambda-hello$ curl http://localhost:3000/
```
...
```bash
lambda-hello$ sam logs -n HelloWorldFunction --stack-name lambda-hello --tail
```
...
```bash
lambda-hello$ pip install -r tests/requirements.txt --user
# unit test
lambda-hello$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
lambda-hello$ AWS_SAM_STACK_NAME=<stack-name> python -m pytest tests/integration -v
```
